        ...
        THEN BEGIN
            ...
            IF essencesLength > 0 AND essencesLength > 1
                THEN BEGIN
                    ...
                    END LOOP;
                END;
                ELSEIF essencesLength > 0 THEN SET saveEssences = JSON_ARRAY(JSON_EXTRACT(essencesPositionArray, "$[0][0]"));
            END IF;
            ...
            IF essencesLength > 0
                THEN BEGIN
                    ...
                    IF CHAR_LENGTH(entitiesString) > 0
                        THEN UPDATE messages SET message_value = entitiesString WHERE message_id = messageID;
                    END IF;
                END;
            END IF;
        END;
...
CREATE DEFINER=`root`@`localhost` FUNCTION `getEssencesInText` (`botID` INT(11), `shift` INT(11), `textNode` TEXT CHARSET utf8) RETURNS JSON NO SQL
BEGIN
  DECLARE responce JSON;
...
CREATE TABLE `bot_essences` (
`essence_id` int(11)
,`essence_value` varchar(1024)
,`bot_id` int(11)
);
...
DROP TABLE IF EXISTS `bot_essences`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bot_essences`  AS  select distinct `e`.`essence_id` AS `essence_id`,`e`.`essence_value` AS `essence_value`,`ee`.`bot_id` AS `bot_id` from (`entity_essences` `ee` join `essences` `e` on((`e`.`essence_id` = `ee`.`essence_id`))) ;
